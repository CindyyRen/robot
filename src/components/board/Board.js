...
export default function Board({ robot }) {
  if (!robot) robot = { x: 2, y: 2, facing: 'NORTH' };
  ...
  function container(x, y, facing) {
    ...
    const divs = Array(tableSize * tableSize)
      .fill()
      .map((_, index) => (
        <div
          key={index}
          className={
            index === active ? `square active ${roateclassName}` : `square `
          }
        >
          <BsRobot />
        </div>
